...
def generate_train_arrays_from_file(lines,batch_size):
    ...
    while 1:
        ...
        for _ in range(batch_size):
            ...
            #trainX    
            name = lines[i]
            img = Image.open(r"dataset_mexico/trainX" + '/' + name)
            img = np.array(img) 
            img = img[:,:,0:3]
            ...
            #trainY
            name = lines[i]
            mask = Image.open(r"dataset_mexico/trainY" + '/' + name)
            mask = np.array(mask)
...
def generate_valid_arrays_from_file(lines,batch_size):
    ...
    while 1:
        ...
        for _ in range(batch_size):
            ...
            name = lines[i]
            img = Image.open(r"dataset_mexico/trainX" + '/' + name)
            img = np.pad(np.array(img), ((0, 120), (0, 20), (0, 0)), 'constant', constant_values=0)
            img = img[:,:,0:3]
            img = img/255
            X_train.append(img)
            ...
            #trainY
            name = lines[i]
            img = Image.open(r"dataset_mexico/trainY" + '/' + name)
            img = np.pad(np.array(img), ((0, 120), (0, 20)), 'constant', constant_values=0)
            seg_labels = np.zeros((HEIGHT,WIDTH,NCLASSES))
            for c in range(NCLASSES):
...
if __name__ == "__main__":
    ...
    lines = os.listdir("dataset_mexico/trainX/")
    sort_lines = []
    for names in lines:
    ...
    file_test=open('dataset_mexico/test_list.txt',mode='w')
    for names in test:
